...
![装工具](img/qs-01-install-tool.svg)
...
不记得命令就敲 `skillpm -h`，最后一行「卸载工具本身」给的就是你这台电脑上能直接复制的命令。
...
Windows（cmd、PowerShell 都能直接跑，不用改执行策略）：

```powershell
~\.skillpm-src\scripts\uninstall.cmd
```

> Windows 上**别跑 `uninstall.sh`**：Windows 不认 `.sh`，只会用记事本之类的程序把脚本打开，什么都没卸。
...
| 装要口令的 Skill（如某个工具的写权限） | `skillpm install <名字>`，在终端里手敲口令（找管理员要） |
